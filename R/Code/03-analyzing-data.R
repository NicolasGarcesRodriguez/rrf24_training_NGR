...
summary_table <- datasummary(
    district*(Mean + SD) ~ livestock_now+ livestock_before+ food_cons_usd+ nonfood_cons_usd,  
    data = hh_data,
...
balance_table <- datasummary_balance(
    hh_size + n_child_5 + n_elder + read + sick + female_head ~ treatment,
    data = hh_data,
    stars = TRUE,
...
model3 <- lm(food_cons_usd_w ~ treatment + crop_damage +drought_flood + district-1, data = hh_data)
...
ggplot(hh_data_plot, aes(x = treatment, y = area_acre_w, fill = treatment)) +
    ...
    theme_minimal() +
    theme(axis.text.x = element_blank(),  # Remove repeated x-axis labels
...
long_data <- secondary_data %>%
    ...
    pivot_longer(cols = c(n_school, n_mededical), names_to = "amenity", values_to = "count") %>%
